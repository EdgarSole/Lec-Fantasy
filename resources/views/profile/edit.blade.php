            ...
            @if (session('status') === 'profile-updated')
                <div x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 3000)"
                    ...
                    <div class="flex items-center">
                        ...
                        <p class="text-green-300 font-medium">{{ __('Perfil actualizado correctamente') }}</p>
                    </div>
                </div>
            @endif
            ...
            @if ($errors->any())
                <div class="p-4 bg-red-900/20 border-l-4 border-red-500 rounded-r-lg">
                    <div class="flex items-center">
                        ...
                        <p class="text-red-300 font-medium">{{ __('Error al actualizar el perfil') }}</p>
                    </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm dark:shadow-gray-900/50 border border-gray-200 dark:border-gray-700 overflow-hidden">
            ...
            <div class="p-8 flex flex-col sm:flex-row items-start gap-6 bg-gradient-to-r from-blue-50 dark:from-gray-700 to-white dark:to-gray-800">
                ...
                <div class="flex-1">
                    ...
                    <p class="text-sm text-gray-500 dark:text-gray-400 mt-2">Miembro desde {{ auth()->user()->created_at->format('d/m/Y') }}</p>
                </div>
            </div>
            ...
            <div class="p-8 space-y-8" x-show="!editMode">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div class="bg-white dark:bg-gray-700 p-5 rounded-lg border border-gray-200 dark:border-gray-600 shadow-sm hover:shadow-md dark:hover:shadow-gray-900/50 transition-shadow">
                        <h3 class="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Ligas Activas</h3>
                        <p class="text-2xl font-bold text-gray-800 dark:text-gray-200 mt-2">{{ $ligasActivas }}</p>
                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Participando actualmente</p>
                    </div>
                    <div class="bg-white dark:bg-gray-700 p-5 rounded-lg border border-gray-200 dark:border-gray-600 shadow-sm hover:shadow-md dark:hover:shadow-gray-900/50 transition-shadow">
                        <h3 class="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Posición Global</h3>
                        <p class="text-2xl font-bold text-gray-800 dark:text-gray-200 mt-2">#{{ $posicionGlobal }}</p>
                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">Top de jugadores</p>
                    </div>
                </div>

                <!-- Descripción -->
                <div class="bg-gray-50 dark:bg-gray-700/50 p-6 rounded-lg border border-gray-200 dark:border-gray-600">
                    <h3 class="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-3">Sobre mí</h3>
            ...
            <div x-show="editMode" class="p-8">
                <form method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data">
                    ...
                    <div class="space-y-6">
                        ...
                        <!-- Foto de perfil -->
                        <div>
                            <label for="foto_url" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Foto de perfil</label>
        ...
        <x-modal name="confirm-user-deletion" :show="$errors->userDeletion->isNotEmpty()" focusable>
            <form method="post" action="{{ route('profile.destroy') }}" class="p-6 bg-white dark:bg-gray-800 rounded-lg shadow-lg text-gray-900 dark:text-gray-100">
                ...
                <div class="text-center">
                    ...
                    <h2 class="text-xl font-bold mt-3 dark:text-gray-200 text-gray-900">¿Eliminar cuenta permanentemente?</h2>
                ...
                <div class="mt-6">
                    <label for="password" class="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">Confirma tu contraseña</label>
